...
setup() {
  ...
  ccmux --no-attach "$TEST_PROJECT" >/dev/null 2>&1
}
...
@test "ccmux-kill fails with no arguments" {
  run ccmux-kill
  [ "$status" -ne 0 ]
  [[ "$output" == *"Usage"* ]]
}

@test "ccmux-kill session kills the entire session" {
...
@test "ccmux-kill session+window kills only that window" {
  local session="${TEST_PREFIX}-kill"

  # Add a worktree window
  ccmux --no-attach "$TEST_PROJECT" kill-feat

  local window_count_before
  window_count_before=$(tmux list-windows -t "$session" | wc -l | tr -d ' ')
  [ "$window_count_before" -eq 2 ]

  run ccmux-kill --yes "$session" kill-feat
  [ "$status" -eq 0 ]
...
@test "ccmux-kill window cleans up git worktree" {
  local session="${TEST_PREFIX}-kill"
...
@test "ccmux-kill fails gracefully for nonexistent session" {
  run ccmux-kill --yes "nonexistent-session-xyz"
  [ "$status" -ne 0 ]
  [[ "$output" == *"not found"* ]]
}
